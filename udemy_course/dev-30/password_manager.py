# An improved Pasword manager app to handle JSON data and search functionality
...
def new_password():
    ...
    pyperclip.copy(
        password
    )  # Copy the password to the clipboard so that the user can paste it into the password field of the website.
...
def validate(username, password, website):
    ...
    if any_errors:
        messagebox.showerror(
            title="Invalid data",
            message=f"Please make sure all fields are filled:\n {message_field}",
        )
...
def write_to_file(website, username, password):
    try:
        ...
        # Set the stream to the start so that it overrides the actual content instead of appending it
        file.seek(0)
    finally:
        # Saving updated data
